...
Loading is **best-effort and never destructive**. Malformed metadata (a null or wrong-typed `version`/`revision`), extra unknown fields, or a file written by a *newer* version of overdue don't stop it loading — the readable activities and logs are recovered individually, anything unreadable is skipped with a warning on stderr, and the file is treated conservatively. Whenever data had to be recovered like this, the **original file is copied to `data.json.bak` before the first change is written**, so a repair or a format downgrade can always be undone by hand. overdue only refuses to load (leaving the file completely untouched) when nothing meaningful can be recovered at all — a file that isn't valid JSON, or an envelope whose `activities` isn't a readable array — so a recoverable problem never becomes data loss.
